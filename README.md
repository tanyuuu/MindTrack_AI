...
切换陪伴者和点击“开始新的对话”都会创建新的独立对话。侧边栏列出历史对话，点击后恢复该对话及其角色；模型只接收当前对话的消息。旧版连续聊天在后端重启时自动迁移到一条历史对话，不删除原记录。此前文档中的“新对话清空旧记录”已不适用。

导出按钮导出当前对话最近 50 条消息；设置中的清空操作仍删除账号的全部消息。

新对话现在采用空白草稿：点击新对话或切换人物不会创建历史条目，发送第一条消息才保存。历史列表隐藏没有消息的旧空对话。每条对话右侧的删除按钮会先确认，再删除该对话及消息；删除当前对话后返回空白草稿。

角色对话复用：切换人物会恢复该角色最近的对话；重复选择当前角色会保留当前已打开的对话。只有从未聊过或原对话已删除时，才在第一条消息发送后创建记录。旧版已产生的多条历史不会自动合并或删除。
