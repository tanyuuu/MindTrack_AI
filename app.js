...
async function detectApi(){try{const r=await fetch("/api/health",{signal:AbortSignal.timeout(1800)});apiOnline=r.ok}catch{apiOnline=false}}
...
async function loadMe(){try{const r=await apiFetch("/api/auth/me");if(!r.ok)throw 0;currentUser=await r.json()}catch{token="";currentUser=null;localStorage.removeItem("mindtrack_token")}}
...
authForm.onsubmit=async e=>{e.preventDefault();await detectApi();if(!apiOnline){showErr("无法连接服务器，请先启动后端或检查部署。");return}authSubmit.disabled=true;try{const r=await fetch(`/api/auth/${authMode}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:authEmail.value.trim(),password:authPassword.value})});const d=await r.json();if(!r.ok)throw new Error(d.detail||"操作失败");if(d.verification_required){pendingVerificationEmail=d.email;verificationEmail.textContent=d.email;authForm.hidden=true;verificationPanel.hidden=false;authTitle.textContent="验证邮箱";authSubtitle.textContent="请输入邮件中的六位验证码（10 分钟内有效）。未收到或已过期？等待 60 秒后重新注册。";authError.hidden=true;verificationCode.focus();return}finishAuth(d)}catch(err){showErr(err.message)}finally{authSubmit.disabled=false;authSubmit.textContent=authMode==="login"?"登录":"创建账号"}}
